...
const BEE_DEFAULT_HOST := "api.hathora.dev"
...
@export var port := BEE_DEFAULT_PORT_HTTPS
...
func log_error(message: String):
	if self.log_level >= LogLevel.ERROR:
		push_error("[HATHORA] "+message)

func log_warning(message: String):
	if self.log_level >= LogLevel.WARNING:
		push_warning("[HATHORA] "+message)

func log_info(message: String):
	if self.log_level >= LogLevel.INFO:
		print("[HATHORA] "+message)
...
# Basic Bearer Authentication `JWT`
func set_security_auth0(value: String):
