...
const ApiConfig = preload("../core/ApiConfig.gd")
const ApiError = preload("../core/ApiError.gd")
...
const BZZ_CONTENT_TYPE_MULTIPART_FORM := "multipart/form-data"
...
const BZZ_PRODUCIBLE_CONTENT_TYPES := [
	...
	BZZ_CONTENT_TYPE_MULTIPART_FORM,
]
...
var _bzz_config: ApiConfig:
	set(value):
		_bzz_config = value
	get:
		if not _bzz_config:
			_bzz_config = ApiConfig.new()
		return _bzz_config
...
func _init(config : ApiConfig = null, client : HTTPClient = null):
	if config != null:
...
func _bzz_request(
	...
	on_success: Callable,  # func(response: ApiResponse)
	on_failure: Callable,  # func(error: ApiError)
):
...
#	print(body) # DEBUG LOG
	...
	if body is Object:
		if body.has_method('bzz_collect_missing_properties'):
			...
			if missing_properties:
				...
				error.message = "%s: is missing required properties %s." % [
					_bzz_name, missing_properties
				]
				on_failure.call(error)
				return
	...
	if content_type == BZZ_CONTENT_TYPE_JSON:
		...
		#Hiding this warning
		#push_warning("Non-seriazable content-type `%s`." % content_type)
		pass
	...
	if not headers.is_empty():
		self._bzz_config.log_debug("→ HEADERS: %s ..(trimmed)" % [str(headers).substr(0,75)])
	if body_serialized:
		if (len(str(body_serialized)) > 300):
			self._bzz_config.log_debug("→ BODY: \n%s ..(trimmed)" % [str(body_serialized).substr(0,300)])
		else:
			self._bzz_config.log_debug("→ BODY: \n%s" % [body_serialized])

	var requesting
	if (body_serialized != null):
		requesting = self._bzz_client.request(method, path_queried, headers_for_godot, body_serialized)
	else:
		requesting = self._bzz_client.request_raw(method, path_queried, headers_for_godot, body_normalized)
	...
	# Perhaps use a method of ApiResponse for this?
	var encoding := "utf-8"
	...
	if not response.headers.is_empty():
		self._bzz_config.log_debug("→ HEADERS: %s ..(trimmed)" % str(response.headers).substr(0,75))
