...
@export var status: String = "":
	set(value):
		if str(value) != "" and not (str(value) in __status__allowable__values):
			push_error("StartingConnectionInfo: tried to set property `status` to a value that is not allowed." +
				"  Allowed values: `starting`")
			return
		__status__was__set = true
		status = value
var __status__was__set := false
var __status__allowable__values := ["starting"]
